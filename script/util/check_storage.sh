...
old_version="script/util/storage_check_cache"
...
if [ -d "script/util/storage_check_report" ]; then
  rm -rf "script/util/storage_check_report"
fi
...
if [ ${#differences[@]} -gt 0 ]; then
    mkdir -p "script/util/storage_check_report"
    printf "%s\n" "${differences[@]}" > "script/util/storage_check_report.removed"
fi
